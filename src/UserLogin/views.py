...
def index(request):
    if request.user.is_authenticated():
        return HttpResponseRedirect(reverse('UserLogin:profile_page', args=(request.user.id,)))
...
def route(request, current_user):
    try:
        ...
        return render(request, 'userLogin/profile.html', {'user_info' : serialized_data})
...
def user_login(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if user:
            login(request, user)
            return HttpResponseRedirect(reverse('UserLogin:profile_page', args=(user.id,)))
...
def sign_up(request):
    ...
    return render(request, 'userLogin/sign_up.html', {'form' : form})
